...
def checkPanAnalysis():
    ...
    try:
        ...
        print(dns_A_ips)
        cprint('[泛解析] {} -> {}'.format(domain, dns_A_ips), 'red')
        return True
    except Exception as e:
        cprint('[不是泛解析] :{}'.format(domain), 'red')
        return False


# 调用Subfindermain脚本
def callSubfindermain():
    cprint('-' * 50 + 'Load Subfindermain ...' + '-' * 50, 'green')
    from Plugins.infoGather.subdomain.subfinder.subfindermain import run_subfindermain
    subfindermains = run_subfindermain(domain)
    return subfindermains
...
def checkCDN_queryA_subdomains(Subdomains_ips, subdomains):
    ...
    for subdomain in subdomains:
        if '.{}'.format(domain) in subdomain:
            tmp_subdomains.append(subdomain.replace('\n', ''))
...
# censysAPI
def run_censysAPI(domain):
    cprint('-' * 50 + 'run_censysAPI ...' + '-' * 50, 'green')  # 启动censys
    from Plugins.infoGather.subdomain.hostCollide import hostCollide
    censysIPS = hostCollide.run_censysAPI(domain)
...
def run_webSpace(domain, SubdomainAndNotCDNIPs, CIP_List, fofaTitle):
    ...
    def run_fofa():
        ...
        if fofaTitle:
            query_str = 'title="{}"'.format(fofaTitle)
            fofa_Results, fofa_web_host_port, fofa_service_host_port = fofaApi.query_domain(query_str)
            if fofa_Results:
                webSpaceSheet.saveWebSpace('fofa', fofa_Results, query_str)  # 将网络空间搜索引擎的结果保存到webSpace项里
                # save_webSpace(fofa_Results, 'fofa', query_str)
    ...
    def run_qianxin():
        # 查询域名
        if domain:
            query_str = 'domain.suffix="{}"'.format(domain)
    ...
    def run_fofaOne(subdomainAndIP_Q):
        while not subdomainAndIP_Q.empty():
            ...
            if fofa_web_host_port:
                ...
                ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
                ip2domainSheet.saveIp2Domain(ip2domain_dict, "fofa反查IP")

    run_fofa()
    # print('1webSpace_web_host_port: {}'.format(webSpace_web_host_port))
    # run_shodan()
    # print('2webSpace_web_host_port: {}'.format(webSpace_web_host_port))
    run_quake()
    # print('3webSpace_web_host_port: {}'.format(webSpace_web_host_port))
    run_qianxin()
    # print('4webSpace_web_host_port: {}'.format(webSpace_web_host_port))
    ...
    # print('5webSpace_web_host_port: {}'.format(webSpace_web_host_port))
...
def get_ip2domain():
    ...
    ip2domain_dict, _newDomains = getIp2Domain.run_ip2domain(domain,
                                                             allTargets_Queue)  # ip2domain_dict字典，key为IP，value为反查的域名
    ...
    ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
    ip2domainSheet.saveIp2Domain(ip2domain_dict, "http://api.webscan.cc/")
...
def collation_web_host(Subdomains_ips, webSpace_web_host_port, ip2domainSubdomains):
    ...
    # print("# 整理开放web服务的host")
    # print(list(set(list(Subdomains_ips.keys()) + ip2domainSubdomains)))
    # print("2222")
    ...
    # print(web_host_port_temp)
    # print("3333")
    # 整合url，格式规范。全部是http(s)://www.domain.com:xxxx
    for host_port in web_host_port_temp:
        if "http" not in host_port:  # 如果没有http（https）， 则加上。如果是443端口，则加https，其他端口加http
            if ':' in host_port:
                try:
    ...
    # print("web_host_port:{}".format(web_host_port))
...
def run_getWebTitle(web_host_port, ip_address_dict):
    cprint('-' * 50 + 'run_getWebTitle ...' + '-' * 50, 'green')  # 筛选存活并获取标题

    from Plugins.infoGather.webInfo import getWebTitle

    if isIntranet == 1:
        threadNum = 10  # 如果是扫内网，则线程为10
    else:
        threadNum = 50  # 扫外网则线程为50
            ...
            try:
                if hasattr(md, 'Detect'):
                    ...
                    for num in range(1, threadNum + 1):
                        t = detect(alive_Web_queue, pbar, webVul_list,
                                   requests_proxies)  # 实例化漏洞类，传递参数：存活web的队列，  存储漏洞的列表
                        threads.append(t)
                        t.start()
                    for t in threads:
                        t.join()
...
def detect_unauthWeakVul(service_host_port):
    ...
    serviceVulName = {'redis': 'unAuthRedis', 'elastic': 'unAuthElastic', 'mongodb': 'unAuthMongodb',
                      'ldaps': 'unAuthLdaps',
...
def printSave_Vul(Vul_list):
    if Vul_list:  # 如果探测出漏洞
        tqdm.write(Fore.WHITE + '-' * 50 + 'Vulnerabilities exist ' + '-' * 50)  # 探测各种漏洞
        for vul in Vul_list:
            Vul_Name, Vul_url, Vul_exist = vul
            tqdm.write(Fore.WHITE + '[{}] {} {}'.format(Vul_Name, Vul_url, Vul_exist))
        ...
        tqdm.write(Fore.WHITE + '-' * 50 + 'Non-existent vulnerabilities' + '-' * 50)
...
def run_subdomain():
    ...
    if not isPanAnalysis and ksubdomain:
        # 0. 调用Subfinder脚本
        ksubdomains = callSubfindermain()
    else:
        ksubdomains = []

    print('[total: {}] subdomain: {}'.format(len(ksubdomains), ksubdomains))
...
'''
    ...
    # 2024-08-02 弃用。没什么用
    censysIPS = run_censysAPI(Subdomains_ips)
...
def run_cSubnet(CIP_List, Subdomains_ips, notCDNSubdomains, param_Links):
    ...
    if len(SubdomainAndNotCDNIPs) > 10:
        counter = Counter(SubdomainAndNotCDNIPs)
        # 按照出现次数从大到小进行排序
        sorted_items = sorted(counter.items(), key=lambda x: x[1], reverse=True)
        # print(sorted_items)
        # 取出前5个元素
        top_five = [item[0] for item in sorted_items[:5]]
        SubdomainAndNotCDNIPs = top_five
    ...
    if domain:  # 跑域名的时候，不跑C段
        ...
        webSpace_web_host_port, webSpace_service_host_port = run_webSpace(domain, [], CIP_List,
                                                                          '')  # 网络空间引擎（fofa、shodan）获取的开放web服务的host（IP/domain）
    ...
    print('[ip2domain get new subdomains] [{}]'.format(
        len(list(set(ip2domainSubdomains) - set(list(Subdomains_ips.keys()))))))
    ...
    for i in web_host_port:
        print("[*] " + i)
    # print("web_host_port:{}".format(web_host_port))
    # print("web_ip_list:{}".format(web_ip_list))
    ...
    # print("web_host_port:{}".format(web_host_port))
    # print("ip_address_dict:{}".format(ip_address_dict))
    web_Title, alive_Web = run_getWebTitle(web_host_port, ip_address_dict)  # 获取C段资产
    # print("web_Title:{}".format(web_Title))
    # print("alive_Web:{}".format(alive_Web))

    # tqdm.write(Fore.WHITE + '-' * 50 + '获取结果' + '-' * 50)
    # for i in web_Title:
    #     print(i)
...
def banner():
  ...
  | () |  \ \ /     / /    / /      / /  
   ...
   本工具由水泽改进而来,原作者为：ske，团队：0x727
   主要更新为：添加ehole的指纹，修复接口api等
'''
    print(banner)
...
def checkVersion():
    ...
    # print("目前版本: \n{}".format(now_version))
    version_url = "https://github.com/xiaocaiji61/ShuiZe_Plus/raw/master/versionFlag.txt"
    headers = {
    ...
    try:
        ...
        if now_version == new_version:
            ...
            cprint("目前版本非最新，建议及时更新...\n地址: https://github.com/xiaocaiji61/ShuiZe_Plus/", 'red')
...
def _init():
    ...
    parse.add_option('-n', '--intranet', dest='isIntranet', type='int', default=0,
                     help='Scan intranet value set to 1')  # 扫描内网, 值为1扫内网， 默认为0
    parse.add_option('-p', '--proxy', dest='proxy', type='string', default=None,
                     help='Intranet proxy socks5 socks4')  # 代理，socks5和socks4, 默认为None，可用于外网扫描，也可以用于内网扫描
    parse.add_option('-f', '--file', dest='File', type='string', default=None,
                     help='/result/2ddcaa3ebbd0/172.18.82.0.xlsx')  # 扫描内网的服务漏洞-未授权和弱口令
    parse.add_option('-w', '--weak', dest='weak', type='int', default=None,
                     help='run weak password script')  # 内网弱口令是否要跑
    ...
    parse.add_option('--mn', dest='masNmapFile', type='str', default=None,
                     help='run masscan nmap result')  # 跑masscan和nmap的结果
    ...
    parse.add_option('--ksubdomain', dest='ksubdomain', type='int', default=1,
                     help='not run ksubdomain')  # 不使用ksubdomain跑子域名
    parse.add_option('--test', dest='testDemo', type='int', default=0, help='if test=1 then run testDemo')  # 测试某个功能
    parse.add_option('--justInfoGather', dest='justInfoGather', type='int', default=0,
                     help='just infoGather, not detect vul')  # 只信息收集，不跑漏洞
    ...
    # 外网代理
        ...
        if kuaidaili_switch == "on":
            requests_proxies = {
                "http": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": kuaidaili_username, "pwd": kuaidaili_password,
                                                                "proxy": kuaidaili_tunnel},
                "https": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": kuaidaili_username, "pwd": kuaidaili_password,
                                                                 "proxy": kuaidaili_tunnel}
            }
            ...
            try:
                kuaidaili_ips = []
                for i in range(3):
                    res = requests.get(url='https://www.taobao.com/help/getip.php', proxies=requests_proxies,
    ...
    if domain and cSubnet:
        ...
        if isIntranet and not web:  # 扫描内网的服务漏洞
            # cprint('-' * 50 + 'Open {} Scanning for service vulnerabilities on the intranet'.format(File) + '-' * 50,
            #        'green')
            # excelSavePath = '{}/{}.xlsx'.format(save_fold_path, excel_name)
        ...
        #
        # ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
        # ip2domainSheet.saveIp2Domain({"1": "123123"}, "test1")
        #
        #
        # ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
        # ip2domainSheet.saveIp2Domain({"2": "123123"}, "test2")
        #
        # ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
        # ip2domainSheet.saveIp2Domain({"3": "123123"}, "test3")
        #
        # ip2domainSheet = saveToExcel(excelSavePath, excel, "IP反查域名")
        # ip2domainSheet.saveIp2Domain({"4": "123123"}, "test4")
