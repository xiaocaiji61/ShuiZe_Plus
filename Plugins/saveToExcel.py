...
class saveToExcel:
    def __init__(self, excelSavePath, excel, title):
        ...
        if title in self.excel.sheetnames:
            # print("写入工作区")
            self.sheet = self.excel.get_sheet_by_name(title)# 读取工作区
            self.Sheet_line = self.sheet.max_row +1
            # print("当前行数：{}".format(self.Sheet_line))
        else:
    ...
    def saveAiqicha(self, selfIcpinfo_infos, invest_infos, holds_infos, branch_infos):
        ...
        for _ in invest_infos:
            pid, invest_info, icp_info, companyDetail_infos = _["pid"], _["invest_info"], _["icp_info"], _[
                "companyDetail_infos"]
        ...
        for _ in holds_infos:
            pid, holds_info, icp_info, companyDetail_infos = _["pid"], _["holds_info"], _["icp_info"], _[
                "companyDetail_infos"]
        ...
        for _ in branch_infos:
            pid, branch_info, icp_info, companyDetail_infos = _["pid"], _["branch_info"], _["icp_info"], _[
                "companyDetail_infos"]
    ...
    # 保存IP反查域名结果
    def saveIp2Domain(self, ip2domain_dict, source):

        if self.Sheet_line == 1:
            self.sheet.cell(self.Sheet_line, 1).value = 'ip'
            self.sheet.cell(self.Sheet_line, 2).value = '域名'
            self.sheet.cell(self.Sheet_line, 3).value = '来源'
            self.Sheet_line += 1
        ...
        for ip in ip2domain_dict.keys():
            ...
            self.sheet.cell(self.Sheet_line, 3).value = source
            self.Sheet_line += 1
