...
def ip2domain(allTargets_Queue, domain, _domain, ip2domain_dict, num, newDomains):
    while not allTargets_Queue.empty():
        ...
        # from Plugins.infoGather.WebspaceSearchEngine import fofaApi
        # query_str = 'ip="{}" && domain!=""'.format(ip)
        # _, fofa_web_host_port, _ = fofaApi.query_ip(query_str)
        # if fofa_web_host_port:
        #     for each in fofa_web_host_port:
        #         tmp_domain = domain.split(".")[-2] +"."+ domain.split(".")[-1]
...
if __name__ == '__main__':
    ...
    allTargets_Queue.put('103.68.175.58')
